...
if ($search_option === 'title') {
  $sql = "SELECT * FROM free_board_table WHERE title LIKE '%$keyword%'";
} elseif ($search_option === 'content') {
  $sql = "SELECT * FROM free_board_table WHERE content = '$keyword'";
} elseif ($search_option === 'name') {
  $sql = "SELECT * FROM free_board_table WHERE name LIKE '%$keyword%'";
} else {
  // 유효하지 않은 검색 조건인 경우 기본 쿼리를 실행
  $sql = "SELECT * FROM free_board_table";
}?>
