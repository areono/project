...
if ($title && $content){
    $sql = mc("UPDATE free_board_table SET title='$title', content='$content', file='$filename' WHERE idx='$bno'");
    if ($sql) {
        echo "<script>alert('글 수정이 완료되었습니다.'); location.href='free_board_detail.php?idx=$bno';</script>";
    } else {
